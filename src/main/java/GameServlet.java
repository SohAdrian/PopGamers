...
public class GameServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String p = request.getParameter("gamePicture");
		String d = request.getParameter("gameDescription");
		String g = request.getParameter("genre");

		// Step 3: attempt connection to database using JDBC, you can change the
		// username and passwordaccordingly using the phpMyAdmin > User Account
		// dashboard
		try {
			...
			PreparedStatement ps = con.prepareStatement("insert into GAMEDETAILS values(?,?,?,?)");
